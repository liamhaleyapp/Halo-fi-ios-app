...
struct EmailVerificationView: View {
    ...
    ZStack {
      ...
      VStack(spacing: 24) {
        VStack(spacing: 16) {
          ...
          Text("We emailed a 6-digit code to \(email). Enter it below to finish creating your account. Check spam if it takes more than a minute.")
            .font(.body)
  ...
  private func verify() {
    ...
    Task {
      ...
      do {
        try await userManager.verifyEmailCode(idUser: idUser, code: code)
        onVerified()
        dismiss()
      } catch {
        errorMessage = "That code didn't work. Check your email and try again."
        Logger.error("Email code verify failed: \(error)")
      }
    }
  }
  ...
  private func resend() {
    ...
    Task {
      ...
      do {
        try await userManager.resendEmailCode(idUser: idUser)
        startResendCooldown()
      } catch {
        errorMessage = "Couldn't resend the code. Try again in a moment."
        Logger.error("Email code resend failed: \(error)")
      }
    }
  }
